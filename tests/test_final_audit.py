...
def test_screen_passes_catches_one_polarisation_and_track_wide_artefacts():
    ...
    for aoi, vh in (("aoi1", -2.6), ("aoi2", -2.2), ("aoi3", -7.5), ("aoi4", -0.3), ("aoi5", None)):
        for date, jvh in (("2026-06-11", vh), ("2026-06-23", 0.2 if vh is not None else None)):
            rows.append({"aoi": aoi, "track": "T1", "pol": "VH", "date": date, "stable_pixels": 0 if vh is None else 50,
                         "stable_jump_db": jvh, "bad": False})
            rows.append({"aoi": aoi, "track": "T1", "pol": "VV", "date": date, "stable_pixels": 0 if vh is None else 50,
    ...
    rows.append({"aoi": "aoi4", "track": "T1", "pol": "VH", "date": "2026-07-05", "stable_pixels": 50, "stable_jump_db": 3.2, "bad": False})
    rows.append({"aoi": "aoi4", "track": "T1", "pol": "VV", "date": "2026-07-05", "stable_pixels": 50, "stable_jump_db": 3.4, "bad": False})
