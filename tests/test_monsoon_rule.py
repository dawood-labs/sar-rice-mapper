...
def _series():
    ...
    ndvi = np.stack([rice, young, trees, bare, flood, cut], axis=1)
...
def test_classes_follow_trough_and_rise():
    ...
    assert mr.classify(ev).tolist() == [3, 2, 0, 0, 0, 4]   # no radar: standing rice unconfirmed; water->soil is no canopy; cut crop is 4
    assert mr.classify(ev, radar_wet=[True] * 6).tolist() == [1, 2, 0, 0, 0, 4]
